...
from utils.web_scraper import prepare_data
...
def train_model(e_val, trainX, trainY, testX, testY):
...
def predict(ticker):
    data_frame = prepare_data(ticker)
    ...
    for e_val in epoch_list:
        model = train_model(e_val=int(e_val), trainX=trainX, trainY=trainY, testX=testX, testY=testY)
...
if __name__ == "__main__":
    ...
    print("Input ticker. (Or simply press enter to skip this process.)")
    ticker = input() if input() else "GOOG"

    predict(ticker=ticker)
